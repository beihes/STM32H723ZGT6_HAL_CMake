...
void LCD_WriteCommand(uint8_t lcd_command)
{
    ...
    HAL_SPI_Transmit(spiLCDData.hspi, &lcd_command, 1, 1000);
}
...
void LCD_WriteData_8bit(uint8_t lcd_data)
{
    ...
    HAL_SPI_Transmit(spiLCDData.hspi, &lcd_data, 1, 1000); // 启动SPI传输
}
...
void LCD_WriteData_16bit(uint16_t lcd_data)
{
    ...
    HAL_SPI_Transmit(spiLCDData.hspi, lcd_data_buff, 2, 1000); // 启动SPI传输
}
...
void LCD_WriteBuff(uint16_t* DataBuff, uint16_t DataSize)
{
    ...
    spiLCDData.hspi->Init.DataSize = SPI_DATASIZE_16BIT; //	16位数据宽度
    HAL_SPI_Init(spiLCDData.hspi);

    HAL_SPI_Transmit(spiLCDData.hspi, (uint8_t*)DataBuff, DataSize, 1000); // 启动SPI传输

    // 改回8位数据宽度，因为指令和部分数据都是按照8位传输的
    spiLCDData.hspi->Init.DataSize = SPI_DATASIZE_8BIT; //	8位数据宽度
    HAL_SPI_Init(spiLCDData.hspi);
}
...
void LCD_SetAddress(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    ...
    LCD_WriteData_16bit(x1 + spiLCDData.X_Offset);
    LCD_WriteData_16bit(x2 + spiLCDData.X_Offset);

    LCD_WriteCommand(0x2b); //	行地址设置，即Y坐标
    LCD_WriteData_16bit(y1 + spiLCDData.Y_Offset);
    LCD_WriteData_16bit(y2 + spiLCDData.Y_Offset);
...
void LCD_SetColor(uint32_t Color)
{
    ...
    spiLCDData.Color = (uint16_t)(Red_Value | Green_Value | Blue_Value); // 将颜色写入全局LCD参数
}
...
void LCD_SetBackColor(uint32_t Color)
{
    ...
    spiLCDData.BackColor = (uint16_t)(Red_Value | Green_Value | Blue_Value); // 将颜色写入全局LCD参数
}
...
{
    spiLCDData.Direction = direction; // 写入全局LCD参数

    if (direction == Direction_H) // 横屏显示
    {
        LCD_WriteCommand(0x36);   // 显存访问控制 指令，用于设置访问显存的方式
        LCD_WriteData_8bit(0x70); // 横屏显示
        spiLCDData.X_Offset = 0;         // 设置控制器坐标偏移量
        spiLCDData.Y_Offset = 0;
        spiLCDData.Width = LCD_Height; // 重新赋值长、宽
        spiLCDData.Height = LCD_Width;
    }
        ...
        spiLCDData.X_Offset = 0;         // 设置控制器坐标偏移量
        spiLCDData.Y_Offset = 0;
        spiLCDData.Width = LCD_Width; // 重新赋值长、宽
        spiLCDData.Height = LCD_Height;
    }
    else if (direction == Direction_H_Flip) {
        LCD_WriteCommand(0x36);   // 显存访问控制 指令，用于设置访问显存的方式
        LCD_WriteData_8bit(0xA0); // 横屏显示，并上下翻转，RGB像素格式
        spiLCDData.X_Offset = 80;        // 设置控制器坐标偏移量
        spiLCDData.Y_Offset = 0;
        spiLCDData.Width = LCD_Height; // 重新赋值长、宽
        spiLCDData.Height = LCD_Width;
    }
...
{
    spiLCDData.asciiFonts = Asciifonts;
}

/****************************************************************************************************************************************
 *	函 数 名:	LCD_Clear
 *
 *	函数功能:	清屏函数，将LCD清除为 spiLCDData.BackColor 的颜色
 *
...
void LCD_Clear(void)
{
    LCD_SetAddress(0, 0, spiLCDData.Width - 1, spiLCDData.Height - 1); // 设置坐标
    ...
    LCD_SPI_Transmit(spiLCDData.hspi, spiLCDData.BackColor, spiLCDData.Width * spiLCDData.Height); // 启动传输
 ...
 *					height - 要清除区域的纵向宽度
 *
 *	函数功能:	局部清屏函数，将指定位置对应的区域清除为 spiLCDData.BackColor 的颜色
 *
...
void LCD_ClearRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height)
{
    ...
    LCD_SPI_Transmit(spiLCDData.hspi, spiLCDData.BackColor, width * height); // 启动传输
...
void LCD_DisplayChar(uint16_t x, uint16_t y, uint8_t c)
{
    ...
    for (index = 0; index < spiLCDData.asciiFonts->Sizes; index++) {
        disChar = spiLCDData.asciiFonts->pTable[c * spiLCDData.asciiFonts->Sizes + index]; // 获取字符的模值
        for (counter = 0; counter < 8; counter++) {
            if (disChar & 0x01) {
                spiLCDData.buffer[i] = spiLCDData.Color; // 当前模值不为0时，使用画笔色绘点
            }
            else {
                spiLCDData.buffer[i] = spiLCDData.BackColor; // 否则使用背景色绘制点
            }
            disChar >>= 1;
            i++;
            w++;
            if (w == spiLCDData.asciiFonts->Width) // 如果写入的数据达到了字符宽度，则退出当前循环
            {                               // 进入下一字符的写入的绘制
                w = 0;
                break;
            }
    ...
    LCD_SetAddress(x, y, x + spiLCDData.asciiFonts->Width - 1, y + spiLCDData.asciiFonts->Height - 1); // 设置坐标
    LCD_WriteBuff(spiLCDData.buffer, spiLCDData.asciiFonts->Width * spiLCDData.asciiFonts->Height);             // 写入显存
}
...
{
    while ((x < spiLCDData.Width) && (*p != 0)) // 判断显示坐标是否超出显示区域并且字符是否为空字符
    {
        LCD_DisplayChar(x, y, *p);
        x += spiLCDData.asciiFonts->Width; // 显示下一个字符
        p++;                        // 取下一个字符地址
    }
}
...
{
    spiLCDData.cnFonts = fonts; // 设置中文字体
    switch (fonts->Width) {
    case 12:
        spiLCDData.asciiFonts = &ASCII_Font12;
        break; // 设置ASCII字符的字体为 1206
    case 16:
        spiLCDData.asciiFonts = &ASCII_Font16;
        break; // 设置ASCII字符的字体为 1608
    case 20:
        spiLCDData.asciiFonts = &ASCII_Font20;
        break; // 设置ASCII字符的字体为 2010
    case 24:
        spiLCDData.asciiFonts = &ASCII_Font24;
        break; // 设置ASCII字符的字体为 2412
    case 32:
        spiLCDData.asciiFonts = &ASCII_Font32;
...
void LCD_DisplayChinese(uint16_t x, uint16_t y, char* pText)
{
    ...
    while (1) {
        ...
        if (*(spiLCDData.cnFonts->pTable + (i + 1) * spiLCDData.cnFonts->Sizes + 0) == *pText && *(spiLCDData.cnFonts->pTable + (i + 1) * spiLCDData.cnFonts->Sizes + 1) == *(pText + 1)) {
            addr = i; // 字模地址偏移
            break;
        }
        i += 2; // 每个中文字符编码占两字节

        if (i >= spiLCDData.cnFonts->Table_Rows)
            break; // 字模列表中无相应的汉字
    }
    i = 0;
    for (index = 0; index < spiLCDData.cnFonts->Sizes; index++) {
        disChar = *(spiLCDData.cnFonts->pTable + (addr)*spiLCDData.cnFonts->Sizes + index); // 获取相应的字模地址
        ...
        for (counter = 0; counter < 8; counter++) {
            ...
            if (Xaddress == spiLCDData.cnFonts->Width) //	如果水平坐标达到了字符宽度，则退出当前循环
            {                                   //	进入下一行的绘制
                Xaddress = 0;
                break;
            }
    ...
    LCD_SetAddress(x, y, x + spiLCDData.cnFonts->Width - 1, y + spiLCDData.cnFonts->Height - 1); // 设置坐标
    LCD_WriteBuff(spiLCDData.buffer, spiLCDData.cnFonts->Width * spiLCDData.cnFonts->Height);             // 写入显存
}
...
void LCD_DisplayText(uint16_t x, uint16_t y, char* pText)
{
    ...
    while (*pText != 0) // 判断是否为空字符
    {
        if (*pText <= 0x7F) // 判断是否为ASCII码
        {
            ...
            x += spiLCDData.asciiFonts->Width;    // 水平坐标调到下一个字符处
            pText++;                       // 字符串地址+1
        }
        else // 若字符为汉字
        {
            LCD_DisplayChinese(x, y, pText); // 显示汉字
            x += spiLCDData.cnFonts->Width;         // 水平坐标调到下一个字符处
...
void LCD_DisplayNumber(uint16_t x, uint16_t y, int32_t number, uint8_t len)
{
    ...
    if (spiLCDData.ShowNum_Mode == Fill_Zero) // 多余位补0
    {
...
void LCD_DisplayDecimals(uint16_t x, uint16_t y, double decimals, uint8_t len, uint8_t decs)
{
    ...
    if (spiLCDData.ShowNum_Mode == Fill_Zero) // 多余位填充0模式
    {
...
void LCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    ...
    for (curpixel = 0; curpixel <= numpixels; curpixel++) {
        LCD_DrawPoint(x, y, spiLCDData.Color); /* Draw the current pixel */
...
void LCD_DrawLine_V(uint16_t x, uint16_t y, uint16_t height)
{
    ...
    for (i = 0; i < height; i++) {
        spiLCDData.buffer[i] = spiLCDData.Color; // 写入缓冲区
    }
    LCD_SetAddress(x, y, x, y + height - 1); // 设置坐标

    LCD_WriteBuff(spiLCDData.buffer, height); // 写入显存
}
...
void LCD_DrawLine_H(uint16_t x, uint16_t y, uint16_t width)
{
    ...
    LCD_WriteBuff(spiLCDData.buffer, width); // 写入显存
}
/***************************************************************************************************************************************
 *	函 数 名: LCD_DrawRect
 *
...
void LCD_DrawCircle(uint16_t x, uint16_t y, uint16_t r)
{
    ...
    do {
        ...
        LCD_DrawPoint(x - Xadd, y + Yadd, spiLCDData.Color);
        LCD_DrawPoint(x + Xadd, y + Yadd, spiLCDData.Color);
        LCD_DrawPoint(x + Xadd, y - Yadd, spiLCDData.Color);
        LCD_DrawPoint(x - Xadd, y - Yadd, spiLCDData.Color);
...
void LCD_DrawEllipse(int x, int y, int r1, int r2)
{
    ...
    if (r1 > r2) {
        do {
            ...
            LCD_DrawPoint(x - Xadd, y + (uint16_t)(Yadd / K), spiLCDData.Color);
            LCD_DrawPoint(x + Xadd, y + (uint16_t)(Yadd / K), spiLCDData.Color);
            LCD_DrawPoint(x + Xadd, y - (uint16_t)(Yadd / K), spiLCDData.Color);
            LCD_DrawPoint(x - Xadd, y - (uint16_t)(Yadd / K), spiLCDData.Color);
        ...
        do {
            ...
            LCD_DrawPoint(x - (uint16_t)(Xadd / K), y + Yadd, spiLCDData.Color);
            LCD_DrawPoint(x + (uint16_t)(Xadd / K), y + Yadd, spiLCDData.Color);
            LCD_DrawPoint(x + (uint16_t)(Xadd / K), y - Yadd, spiLCDData.Color);
            LCD_DrawPoint(x - (uint16_t)(Xadd / K), y - Yadd, spiLCDData.Color);
...
void LCD_FillRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height)
{
    ...
    LCD_SPI_Transmit(spiLCDData.hspi, spiLCDData.Color, width * height);
...
void LCD_DrawImage(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t* pImage)
{
    ...
    // 因为缓冲区大小有限，需要分多次写入
    Buff_Height = (sizeof(spiLCDData.buffer) / 2) / height; // 计算缓冲区能够写入图片的多少行
    ...
    for (i = 0; i < height; i++) // 循环按行写入
    {
        for (j = 0; j < (float)width / 8; j++) {
            ...
            for (m = 0; m < 8; m++) {
                if (disChar & 0x01) {
                    spiLCDData.buffer[BuffCount] = spiLCDData.Color; // 当前模值不为0时，使用画笔色绘点
                }
                else {
                    spiLCDData.buffer[BuffCount] = spiLCDData.BackColor; // 否则使用背景色绘制点
                }
                disChar >>= 1;               // 模值移位
                Xaddress++;                  // 水平坐标自加
                BuffCount++;                 // 缓冲区计数
        ...
        if (BuffCount == Buff_Height * width) // 达到缓冲区所能容纳的最大行数时
        {
            ...
            LCD_WriteBuff(spiLCDData.buffer, width * Buff_Height);                           // 写入显存
        ...
        if ((i + 1) == height) // 到了最后一行时
        {
            ...
            LCD_WriteBuff(spiLCDData.buffer, width * (i + 1 + y - Yaddress)); // 写入显存
        }
    }
}
...
void LCD_CopyBuffer(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t* DataBuff)
{
    ...
    LCD_SPI_TransmitBuffer(spiLCDData.hspi, DataBuff, width * height);
...
char midSPILCDStr[256] = "";
void SPILCD_Printf(int x, int y, const char* format, ...)
{
    uint16_t length = 0;
    va_list args;
    va_start(args, format);
    length = vsnprintf(midSPILCDStr, sizeof(midSPILCDStr), (char*)format, args);
    midSPILCDStr[length] = '\0';
    LCD_DisplayText(x, y, midSPILCDStr);
}