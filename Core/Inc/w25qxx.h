...
#define W25Qxx_NumByteToTest 32 * 1024 // 测试数据的长度，32K
    extern uint32_t w25qxxID;// 器件ID
    extern uint8_t w25qxxState;//是否正常
#endif

    uint8_t OSPI1_W25Qxx_Init(void);     // W25Qxx初始化
    uint32_t OSPI1_W25Qxx_ReadID(void); // 读取器件ID

    uint8_t OSPI1_W25Qxx_MemoryMappedMode(void); // 将OSPI设置为内存映射模式

    uint8_t OSPI1_W25Qxx_SectorErase(uint32_t SectorAddress);    // 扇区擦除，4K字节， 参考擦除时间 45ms
    uint8_t OSPI1_W25Qxx_BlockErase_32K(uint32_t SectorAddress); // 块擦除，  32K字节，参考擦除时间 120ms
    uint8_t OSPI1_W25Qxx_BlockErase_64K(uint32_t SectorAddress); // 块擦除，  64K字节，参考擦除时间 150ms，实际建议使用64K擦除，擦除的时间最快
    uint8_t OSPI1_W25Qxx_ChipErase(void);                        // 整片擦除，参考擦除时间 20S

    uint8_t OSPI1_W25Qxx_WritePage(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite); // 按页写入，最大256字节
    uint8_t OSPI1_W25Qxx_WriteBuffer(uint8_t* pBuffer, uint32_t WriteAddr, uint32_t Size);         // 写入数据，最大不能超过flash芯片的大小
    uint8_t OSPI1_W25Qxx_ReadBuffer(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead);  // 读取数据，最大不能超过flash芯片的大小
